...
const addReaction = async (messageId: string, emoji: string) => {
  const response = await fetch(`/api/messages/messages/${messageId}/reactions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    ...
    >
      ...
      <div className={`flex flex-col max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
        ...
        >
          ...
          {/* Message Actions */}
          <div className={`absolute top-0 ${isOwn ? '-left-20' : '-right-20'} opacity-0 group-hover:opacity-100 transition-opacity`}>
            ...
            <div className="flex items-center gap-4">
              ...
              <Avatar className="h-10 w-10">
                <AvatarImage 
                  ...
                  alt={chat.name || otherParticipant?.displayName || ''} 
                />
                <AvatarFallback>
                  {chat.name?.charAt(0) || otherParticipant?.displayName?.charAt(0) || '?'}
                </AvatarFallback>
              </Avatar>
          ...
          {replyTo && (
            <div className="mb-3 p-3 bg-muted/50 rounded-lg border-l-4 border-primary">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-muted-foreground">Replying to {replyTo.sender.displayName || replyTo.sender.username}</p>
